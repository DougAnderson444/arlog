...
import { arlog, arLogLoaded, portal, selectedNetwork } from './stores.js';
let dev = import.meta.env.DEV || true; //false;
const LIVE_NET = 'LIVE_NET';
const DEV_NET = 'DEV_NET';
let allLogs;
let contractIDs = [];
...
$: if ($portal && $arLogLoaded && configArweaveWallet)
    configArweaveWallet();
onMount(async () => {
...
async function showLogs() {
    if ($selectedNetwork !== config.networks.MAIN_NET.name)
        await $arlog.doMining();
